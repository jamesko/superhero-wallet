...
import { checkAddressOrChannel, handleUnknownError } from '../../utils/helper';
...
export default {
  components: {
    Pending, Avatar, Truncate, InputField, ButtonPlain, DetailsItem, Arrow, Save, Paste, HintButton,
  },
  props: {
  ...
  methods: {
    ...
    async setAutoExtend() {
      if (!this.autoExtend) {
        await this.$store.dispatch('modals/open', {
          name: 'confirm',
          icon: 'info',
          title: this.$t('pages.names.list.autoextend-confirm-title'),
          msg: this.$t('pages.names.list.autoextend-confirm-msg'),
        });
      }
    ...
    async readValueFromClipboard() {
      switch (process.env.PLATFORM) {
        case 'cordova':
          this.newPointer = await new Promise((...args) => window
            .cordova.plugins.clipboard.paste(...args));
          break;
        case 'extension':
          this.newPointer = await browser.runtime.sendMessage({ method: 'paste' });
          break;
        default:
          try {
            this.newPointer = await navigator.clipboard.readText();
          } catch (e) {
